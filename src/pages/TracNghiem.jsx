...
export default function TracNghiem() {
  ...
  const exportQuizToPDF = async () => {
    const input = document.getElementById("quiz-pdf");
    ...
    pdf.save(`${studentInfo.name}_ketqua.pdf`);
  };
    ...
    const fetchQuestions = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          saved = shuffleArray(saved); // Shuffle toàn bộ câu hỏi lúc load
          ...
          loadedQuestions = loadedQuestions.filter(q => {
            ...
            if (q.type === "single" || q.type === "multiple" || q.type === "image") return q.question.trim() !== "" && q.options.length > 0 && Array.isArray(q.correct);
            if (q.type === "truefalse") return q.question.trim() !== "" && q.options.length >= 2 && Array.isArray(q.correct);
            return false;
          });

          // Đảo thứ tự câu hỏi hình ảnh
          const imageQuestions = loadedQuestions.filter(q => q.type === "image");
          const otherQuestions = loadedQuestions.filter(q => q.type !== "image");
          loadedQuestions = [...otherQuestions, ...shuffleArray(imageQuestions)];
        }
  ...
  const handleSubmit = async () => {
    ...
    // 🔍 Kiểm tra câu chưa trả lời: single/multiple/image/truefalse
    ...
    try {
      ...
      questions.forEach(q => {
        ...
        if (q.type === "single") {
          ...
          if (typeof ua === "number" && correctArray.includes(ua)) total += q.score ?? 1;
        }
        ...
        // 🔹 MULTIPLE
          ...
          if (userSet.size === correctSet.size && [...correctSet].every(x => userSet.has(x))) {
            total += q.score ?? 1;
          }
        }
        ...
        // 🔹 SORT
          ...
          const userAnswerArray = Array.isArray(rawAnswer) ? rawAnswer : [];
          if (userAnswerArray.length > 0 && userAnswerArray.every((val, i) => val === q.correct[i])) {
            total += q.score ?? 1;
          }
        }
        ...
        // 🔹 TRUE/FALSE
          ...
          const userAnswerArray = Array.isArray(rawAnswer) ? rawAnswer : [];
          if (userAnswerArray.length === q.correct.length &&
              userAnswerArray.every((val, i) => val === q.correct[i])) {
            total += q.score ?? 1;
          }
        }
      });
      ...
      const durationSec = config?.timeLimit ? (config.timeLimit * 60 - timeLeft) : 0;
      const durationStr = formatTime(durationSec); // ví dụ "07:13"

      // Gọi hàm PDF với tham số durationStr
      exportQuizPDF(studentInfo, quizClass, questions, answers, total, durationStr);
  ...
  const autoSubmit = async () => {
    ...
    try {
      ...
      questions.forEach(q => {
        ...
        // IMAGE (hình ảnh multiple choice)
        else if (q.type === "image") {
          const userArray = Array.isArray(rawAnswer) ? rawAnswer.map(Number) : [];
          const correctArray = Array.isArray(q.correct) ? q.correct : [q.correct];
          const correctSet = new Set(correctArray);
      ...
      {!loading && currentQuestion && (
        <>
          ...
          {currentQuestion.type === "image" && (
            <Stack
              direction="row"
              spacing={4} // khoảng cách giữa các ô
              flexWrap="wrap"
                ...
                return (
                  ...
                  >
                    <img
                      ...
                      alt={`option ${optIdx + 1}`}
                      style={{ maxHeight: 80, maxWidth: "100%", objectFit: "contain", marginBottom: 4 }}
                    />
                    <Checkbox
                      checked={checked}
                      disabled={submitted || !started}
                      onChange={() => handleMultipleSelect(currentQuestion.id, optIdx, !checked)}
                      sx={{
                        color: !submitted
                          ? undefined
                          : isCorrect
                          ? "#388e3c" // hoặc có thể dùng màu xanh đậm để tương phản tốt trên nền #c8e6c9
                          : isWrong
                          ? "#d32f2f" // đỏ đậm để tương phản nền #ffcdd2
                          : undefined,
                        "&.Mui-checked": {
                          color: !submitted
                            ? undefined
