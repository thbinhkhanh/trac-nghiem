...
export const syncLamVanBenToKTDK = async ({
  ...
  try {
    ...
    const generateRandomId = customAlphabet("0123456789", 9);
        ...
        try {
          ...
          await Promise.all(
            studentSnap.docs.map((studentDoc) => {
              ...
              const targetId = /^\d+$/.test(studentDoc.id)
                ? studentDoc.id
                : generateRandomId();
              ...
              const fixedData = {
                ...data,
                lyThuyet: data.diem ?? null,
              };

              delete fixedData.diem;

              return setDoc(targetRef, fixedData, { merge: true });
            })
          );
